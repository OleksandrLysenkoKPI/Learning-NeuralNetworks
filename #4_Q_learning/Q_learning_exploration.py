...
def choose_move(curr_pos, R, Q, chosen_moves, epsilon=0.2,size=5):
    x, y = curr_pos
    ...
    # Випадковий вибір з імовірністю epsilon (дослідження)
    if random.uniform(0, 1) < epsilon:
        chosen_index = random.choice(available_moves)
    else:
        # Жадібний вибір (експлуатація)
...
# Додатково: функція для друку шляху на дошці
...
# Змінні для відстеження найкоротшого епізоду
shortest_path = []
min_steps = float('inf')
...
# Виведення найкоротшого шляху на дошці
print(f"\nShortest path found in {min_steps} steps:")
print_path_on_board(shortest_path)